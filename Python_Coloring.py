...
class Highlighter(QSyntaxHighlighter):
    ...
    def __init__(self, document):
        ...
        pythonRules += [(r'\b%s\b' % w, 0, STYLES['keyword'])
                  for w in Highlighter.pythonKeywords]
        pythonRules += [(r'%s' % o, 0, STYLES['operator'])
                  for o in Highlighter.pythonOperators]
        pythonRules += [(r'%s' % b, 0, STYLES['brace'])
                  for b in Highlighter.pythonBraces]
        ...
        cSharpRules = []
        # Keyword, operator, and brace rules
        cSharpRules += [(r'\b%s\b' % w, 0, STYLES2['keyword'])
                  for w in Highlighter.cSharpKeywords]
        cSharpRules += [(r'%s' % o, 0, STYLES2['operator'])
                  for o in Highlighter.cSharpOperators]
        cSharpRules += [(r'%s' % b, 0, STYLES['brace'])
                  for b in Highlighter.cSharpBraces]
        # All other rules
        cSharpRules += [
            # 'self'
